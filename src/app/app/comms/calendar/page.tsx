...
export default async function CommsCalendarPage({
  ...
  const view = VALID_VIEWS.has(params.view ?? '') ? (params.view as 'month' | 'list') : 'month'
  const statusFilter =
  ...
  const [{ data: entriesData }, { data: authorsData }, { data: intakeCandidatesData }] = await Promise.all([
    ...
    supabase
      .from('profiles')
      .select('id, name, email, role, comms_team')
  ...
  const authors = ((authorsData ?? []) as Array<{
    ...
    .filter((author) => author.role === 'PlatformAdmin' || (author.role === 'Moderator' && author.comms_team))
    .map((author) => ({
      id: author.id,
